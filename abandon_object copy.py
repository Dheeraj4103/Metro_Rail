...
file = os.listdir(os.path.join(os.getcwd(),'videos1','Left_Object'))[3]      
...
while cap.isOpened():
    ...
    if 'box' in columns.tolist():
        detections = detection(results)
        dets = np.array(detections)

        track = tracker.update(dets, frame)
        abandoned_objects = BagDetection.check_abandon(track, 100, list_of_bags=listOfObjects)
